...
//!
//! - One small kernel: `bmma_xor_popc_8x8x128_smoke`. Computes the **Q = 8, dim_bits = 128**
//!   Hamming-distance matrix vs an arbitrary `N` corpus, single warp per 8-corpus tile.
//! - One smoke test (`bmma_smoke_byte_equal_8x1024x128`) that asserts byte-equality vs the CPU
//!   oracle on `Q = 8, N = 1024, dim_bits = 128` — the parity gate the prompt's plan calls out as
//!   the minimum to confirm the PTX path is viable.
//!
//! ## What this module does *not* yet do
//!
//! - Dispatch BMMA for arbitrary `dim_bits ≠ 128`, `Q ≠ 8`, or compute a corpus-wide kernel. Those
//!   need K-loop accumulation, query tiling, and a full launch grid — the prompt's "Phase 5-3
//!   corpus migration" stage. The smoke kernel intentionally stays minimal so a positive parity
//!   result is unambiguous.
//! - Replace the INT8 path. The INT8 IMMA kernels in `kernel.rs` stay the production path; BMMA is
//!   opt-in via the smoke test today and a follow-up `Phase 5-3 a` migrates the corpus path only
//!   after the PoC passes parity at this scale.
...
/// m8n8k128.b1):
/// - **A** (row-major .row): thread `t` holds bits `A[t/4, (t%4)*32 .. (t%4)*32 + 32]`, packed as
///   one `b32`.
/// - **B** (col-major .col): thread `t` holds bits `B[(t%4)*32 .. (t%4)*32 + 32, t/4]`, packed as
///   one `b32`. Mapped to our row-major corpus storage, that means thread `t` loads
///   `corpus[d_tile*8 + t/4, (t%4)*32 .. + 32]`.
/// - **D** (s32 accumulator): thread `t` holds two s32 values at `D[t/4, (t%4)*2]` and `D[t/4,
///   (t%4)*2 + 1]`.
...
pub(super) fn compute_batched_bmma(
    ...
    let mut out_dev = unsafe { stream.alloc::<u32>(num_queries * num_vecs) }.map_err(map_drv)?;
    ...
    fn bmma_smoke_byte_equal_8x1024x128() {
        ...
        let kernels = match build_kernels(&ctx) {
            ...
            Err(e) => {
                ...
                eprintln!("skipping BMMA smoke — NVRTC could not build inline-PTX kernel ({e})");
                return;
            }
        };
        ...
        assert_eq!(
            gpu,
            cpu,
            "BMMA smoke parity violated at Q={q} N={n} dim_bits={dim_bits} — first divergence at \
             index {}",
            gpu.iter()
                .zip(cpu.iter())
                .position(|(a, b)| a != b)
    ...
    fn bmma_general_parity_sweep() {
        ...
        for &(q, n, dim_bits) in cases {
            ...
            assert_eq!(
                gpu,
                cpu,
                "BMMA general parity violated at Q={q} N={n} dim_bits={dim_bits} — first \
                 divergence at index {}",
                gpu.iter()
                    .zip(cpu.iter())
                    .position(|(a, b)| a != b)
