...
//! distance matrix:
//!
//! 1. **GEMM**: `inner[m, n] = Σ_k Q[m,k] * D[n,k]` with INT8 inputs and INT32 accumulation.
//!    `cublasLtMatmul` selects an IMMA tensor-core algorithm via the heuristic for any K ≥ 4 (we
//!    only call with K = `dim_bits` ∈ {32, 64, …, 2048}, all multiples of 32).
//! 2. **Correction**: `out[m, n] = pop_q[m] + pop_d[n] − 2 · inner[m, n]` written by an
//!    NVRTC-compiled element-wise kernel. The output is `u32`, matching the WGSL/CPU oracle.
//!
...
///
/// 1. `unpack_bits` — read row-major `num_rows × dim_u32` u32 input and write row-major `num_rows ×
///    dim_bits` i8 output, with each output byte equal to the matching LSB-first bit. Bits beyond
///    `dim_bits` in the last u32 of each row are dropped (matches the calling-contract assumption
///    that trailing padding bits are zero, which is required by the GEMM-vs-popcount equivalence).
/// 2. `pop_correction` — close the popcount-identity: `out[m, n] = pop_q[m] + pop_d[n] − 2 ·
///    inner[m, n]`. The output is `u32` to match the WGSL kernel and the CPU reference exactly.
///
...
impl CudaGemmRunner {
    ...
    fn pipeline_slots(&self) -> GpuResult<&[PipelineSlot; 2]> {
        ...
        Ok(self.pipeline_slots.get().expect("pipeline_slots set above"))
    }
    ...
    fn gemm_and_correct_dev_on(
        ...
        // - A := corpus (D), shape K × N column-major (= storage row-major N × K), opA = T → op(A)
        //   = D in row-major view.
        // - B := queries (Q), shape K × M column-major (= storage row-major M × K), opB = N → op(B)
        //   = Q^T in column-major view.
        // - Output dim: M_p = N (rows), N_p = M (cols), ldc = N.
        {
            ...
            unsafe {
                self.matmul_int8_int32_on(
                    stream, workspace, a_ptr, b_ptr, c_ptr,    // m_p
                    n as u64, // n_p
                    m as u64, // k_p
                    k as u64, // transa
                    true,     // transb
    ...
    pub(super) fn run_batches_with_cached_corpus_into_pinned(
        ...
        if out.len() < num_batches * result_per_batch {
            return Err(GpuError::ColumnTypeMismatch {
                expected: format!("pinned out buffer len ≥ {}", num_batches * result_per_batch),
                actual: format!("pinned out buffer len = {}", out.len()),
            });
        }
        ...
        for batch_idx in 0..num_batches {
            ...
            let queries_slice = &queries_concat[q_offset..q_offset + queries_per_batch_words];
            ...
            let q_i8_dev = self.unpack_to_device_on(
                &slot.stream,
                queries_slice,
    ...
    pub(super) fn knn_search_with_cached_corpus(
        ...
        let mut top_dists_dev = unsafe { stream.alloc::<u32>(num_queries * k) }.map_err(map_drv)?;
        let mut top_ids_dev = unsafe { stream.alloc::<u32>(num_queries * k) }.map_err(map_drv)?;
    ...
    unsafe fn matmul_int8_int32_on(
        ...
        let c_layout = blas_result::create_matrix_layout(i32_t, m_p, n_p, ldc).map_err(map_blas)?;

        let matmul_desc = blas_result::create_matmul_desc(compute_t, scale_t).map_err(map_blas)?;
        ...
        .map_err(|e| GpuError::CpuFallback {
            reason: format!(
                "no cuBLASLt INT8 algorithm matched the requested shape (M={m_p} N={n_p} K={k_p}, \
                 transa={transa}, transb={transb}): {e}"
            ),
        })?;
