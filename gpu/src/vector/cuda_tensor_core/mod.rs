...
//!
//! 1. `inner = Q · Dᵀ`  via `cublasLtMatmul` with `CUDA_R_8I` inputs and `CUDA_R_32I` accumulator
//!    (compute type `CUBLAS_COMPUTE_32I`).
//! 2. `out[m, n] = pop_q[m] + pop_d[n] − 2 · inner[m, n]` via a small NVRTC-compiled element-wise
//!    kernel.
//!
...
//!
//! - `BinaryDistanceKernel::compute_batched` first tries to construct a [`CudaTensorCoreKernel`].
//!   If construction succeeds (CUDA driver + cuBLASLt + an NVIDIA device + NVRTC are all available
//!   at runtime) it routes the dispatch through this module.
//! - Construction failure (no CUDA, no NVIDIA device, missing libraries, container without
//!   `/dev/nvidia*`) returns [`GpuError::CpuFallback`](crate::error::GpuError::CpuFallback) with a
//!   non-fatal reason; the calling code falls back to the WGSL pipeline.
//! - The construction result is cached (`OnceLock`) for the life of the `BinaryDistanceKernel`, so
    ...
    pub fn compute_batched_bmma(
        ...
        let kernels = match self.bmma_kernels.get() {
            ...
            None => {
                ...
                self.bmma_kernels.get().expect("bmma kernels just set")
            }
        };
...
impl CudaBinaryCorpus {
    ...
    pub fn compute_batched(&self, queries_bits: &[u32], num_queries: usize) -> GpuResult<Vec<u32>> {
    ...
    /// - All batches use the same `num_queries_per_batch` (to keep the cuBLASLt heuristic stable
    ///   across the loop).
    ///
    ...
    pub fn compute_batches_into_pinned(
        ...
        self.inner
            .runner
            .run_batches_with_cached_corpus_into_pinned(
                queries_concat,
                &pop_q_concat,
                &self.inner,
