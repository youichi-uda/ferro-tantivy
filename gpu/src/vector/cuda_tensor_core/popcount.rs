...
//!
//! 1. **Bit unpacking** — turn each bit of a `dim_u32` × N corpus into one `i8` byte (∈ {0, 1}) so
//!    it can be fed to an INT8 cuBLAS GEMM.
//! 2. **Per-vector popcount** — sum the `count_ones()` of each row's u32 words. Required by the
//!    popcount-identity reformulation `popcount(q ⊕ d) = popcount(q) + popcount(d) − 2·⟨q, d⟩`.
//!
//! Both run on the host because:
//! - Unpacking has zero arithmetic intensity (memcpy-bound) — pushing it to the device just doubles
//!   PCIe traffic.
//! - Per-vector popcount is `O(dim_u32 × N)` but each `count_ones()` is a single x86 `popcnt`
//!   cycle, so the host runs at ~30 GB/s effective. Wall-clock cost on a 1 M corpus at dim_bits =
//!   768 is ~3 ms — small compared to the GEMM call.
