...
///
/// Grid:
/// - Q ∈ {1, 4, 32, 128}        — covers single-query and IMMA's M=128 tile boundary
/// - N ∈ {1, 100, 1024, 16_384} — covers small-batch overhead and arithmetic-intensity sweet spot
...
fn parity_sweep_byte_equal() {
    ...
    for &q in q_vals {
        for &n in n_vals {
            for &dim_bits in dim_vals {
                ...
                let d_seed =
                    (0xfedc_ba98u64) ^ ((q as u64) << 16) ^ ((n as u64) << 4) ^ (dim_bits as u64);
                ...
                if gpu != cpu {
                    let first_diff = gpu
                        ...
                        .unwrap();
                    panic!(
                        "byte-equal violated at Q={q} N={n} dim_bits={dim_bits}, first diff \
                         idx={first_diff}: gpu={} cpu={}",
    ...
    eprintln!(
        "CUDA Tensor Core parity sweep: {total_cells} cells passed, max Q={max_q_used}, max \
         N={max_n_used}"
    );
}
...
fn parity_extremes_zero_vs_one() {
    ...
    for (i, &d) in gpu.iter().enumerate() {
        assert_eq!(
            d, dim_bits as u32,
            "all-zero vs all-one mismatch at idx {i}"
        );
    }
}
...
fn parity_cached_corpus_matches_fresh() {
    ...
    for &(q, n, dim_bits) in cases {
        ...
        assert_eq!(
            cached, fresh,
            "cached != fresh at Q={q} N={n} dim={dim_bits}"
        );
    }
}
...
fn parity_pinned_buffer_matches_vec_path() {
    ...
    let cases: &[(usize, usize, usize)] =
        &[(1, 1, 32), (4, 100, 256), (8, 1024, 768), (64, 8192, 1024)];

    for &(q, n, dim_bits) in cases {
        let dim_u32 = dim_u32_for(dim_bits);
        let mut queries =
            random_u32_vec(q * dim_u32, 0x9999_8888_7777_6666u64 ^ q as u64 ^ n as u64);
        let mut corpus =
            random_u32_vec(n * dim_u32, 0xaaaa_bbbb_cccc_ddddu64 ^ q as u64 ^ n as u64);
        zero_padding(&mut queries, q, dim_bits);
        zero_padding(&mut corpus, n, dim_bits);
        ...
        let cached = kernel
            .prepare_corpus(&corpus, n, dim_bits)
            .expect("prepare_corpus");
        let mut pinned = kernel.alloc_pinned_u32(q * n).expect("alloc_pinned_u32");
        cached
        ...
        if q2 > 0 {
            let cpu2 =
                hamming_distances_batched_cpu(&queries[..q2 * dim_u32], &corpus, q2, n, dim_u32);
            cached
...
fn parity_double_buffered_batches_match_single_path() {
    ...
    for &(num_batches, q, n, dim_bits) in cases {
        ...
        let queries_seed = 0xfeed_face_u64
            ...
            ^ dim_bits as u64;
        let corpus_seed =
            0xc0de_d00d_u64 ^ ((num_batches as u64) << 32) ^ ((n as u64) << 16) ^ dim_bits as u64;
        ...
        for b in 0..num_batches {
            let qslice = &queries_concat[b * words_per_batch..(b + 1) * words_per_batch];
            cached
            ...
            assert_eq!(
                lhs, rhs,
                "double-buffered batch {b} != single-batch path (num_batches={num_batches}, \
                 Q={q}, N={n}, dim={dim_bits})"
            );
        }
    }
}
...
fn parity_small_odd_shapes() {
    ...
    for &(q, n, dim_bits) in cases {
        ...
        assert_eq!(
            gpu, cpu,
            "Q={q} N={n} dim_bits={dim_bits} byte-equal violated"
        );
    }
}
