...
//!   18-1).
//! * **Cursor-missing** segments take a brute-force fallback that collects every matched doc and
//!   materialises the per-field sort tuple from the segment's fast-field columns at harvest time.
//!
...
impl EarlyTermOrFallbackCollectorMulti {
    ...
    pub fn new(fields: Vec<(impl Into<String>, Order)>, limit: usize) -> Self {
        Self {
            fields: fields.into_iter().map(|(n, o)| (n.into(), o)).collect(),
            limit,
            start_after: None,
            scoring: None,
        }
    ...
    /// * Cursor branch — the inner v2 segment collector flips `requires_scoring=true` and appends
    ///   `Score(_)` to its fruit.
    /// * Fallback branch — the brute-force segment collector also captures per-doc scores during
    ///   `collect(doc, score)` and appends `Score(_)` to its harvest tuples.
    ///
...
impl FallbackSegmentCollector {
    ...
    fn harvest(self) -> Vec<(Vec<CursorSortVal>, DocAddress)> {
        ...
        for &doc in &self.matched_docs {
            ...
            for fi in 0..prefix_len {
                let val =
                    self.field_readers[fi].read(self.field_kinds[fi], doc, &mut decode_scratch);
                tuple.push(val);
            }
...
mod tests {
    ...
    fn all_cursor_segments_match_wave_18_1_dispatch() {
        ...
        let collector = EarlyTermOrFallbackCollectorMulti::new(vec![("country", Order::Asc)], 10);
    ...
    fn fallback_numeric_multi_field() {
        ...
        assert_eq!(
            pairs,
            vec![
                (200i64.to_u64(), 5i64.to_u64()),
                (100i64.to_u64(), 3i64.to_u64()),
                (100i64.to_u64(), 7i64.to_u64())
            ]
        );
    }
    ...
    fn fallback_with_scoring_appends_score_slot() {
        ...
        let collector = EarlyTermOrFallbackCollectorMulti::new(vec![("ts", Order::Desc)], 10)
        ...
        assert!(
            ...
            "ts=100 tie group: 3× term doc score ({score_doc_1}) must exceed 1× term doc score \
             ({score_doc_0})"
        );
    }
}
