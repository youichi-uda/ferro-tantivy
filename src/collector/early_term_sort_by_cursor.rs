...
//! 2. At [`SegmentCollector::harvest`] time we walk [`SortCursorIndex::iter`] in sort order, and
//!    for each cursor doc:
//!    * skip if the matched bitset does not contain it (i.e. the doc did not match the query, or
//!      was deleted at search time);
//!    * otherwise resolve the sort key from the fast field column and push `(Option<T>,
//!      DocAddress)` into the per-segment fruit;
...
impl<T: FastValue> Collector for EarlyTermSortByCursorCollector<T> {
    ...
    fn merge_fruits(
        ...
        let mut all: Vec<(Option<T>, DocAddress)> = segment_fruits.into_iter().flatten().collect();
...
mod tests {
    ...
    fn search_after_desc_skips_at_or_above_cursor() -> crate::Result<()> {
        ...
        let collector =
            EarlyTermSortByCursorCollector::<i64>::new("v", Order::Desc, 5).with_search_after(95);
        let hits: Vec<(Option<i64>, DocAddress)> = searcher.search(&AllQuery, &collector)?;
        assert_eq!(hits.len(), 5);
    ...
    fn search_after_asc_skips_at_or_below_cursor() -> crate::Result<()> {
        ...
        let collector =
            EarlyTermSortByCursorCollector::<i64>::new("v", Order::Asc, 5).with_search_after(50);
        let hits: Vec<(Option<i64>, DocAddress)> = searcher.search(&AllQuery, &collector)?;
        let got: Vec<i64> = hits.iter().map(|(v, _)| v.unwrap()).collect();
    ...
    fn search_after_skips_missing_value_docs() -> crate::Result<()> {
        ...
        let c1 =
            EarlyTermSortByCursorCollector::<i64>::new("v", Order::Desc, 100).with_search_after(30);
