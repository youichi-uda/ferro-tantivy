...
//!    [`default_collect_segment_impl`](crate::collector::default_collect_segment_impl), which calls
//!    [`SegmentCollector::collect`] / `collect_block` for every alive matching doc.  We record
//!    those docs into a per-segment [`BitSet`].
//! 2. At [`SegmentCollector::harvest`] time we walk the v2 cursor in lex sort order, and for each
//!    cursor doc:
//!    * skip if the matched bitset does not contain it (the doc did not match the query, or was
//!      deleted at search time);
//!    * resolve the per-field encoded sort tuple from the cursor and decode `ValueKind::String`
//!      slots from segment-local term ords to UTF-8 bytes (via the captured per-field `StrColumn`);
//!    * (optional) skip if a `search_after` cursor was supplied and the tuple is at-or-before the
//!      cursor in lex order (kind-aware: u64 cmp for numerics, `Vec<u8>` cmp for strings);
//!    * push `(Vec<CursorSortVal>, DocAddress)` into the per-segment fruit;
...
impl EarlyTermSortByCursorCollectorMulti {
    ...
    ///
    /// **Caveats.**
    /// * The cursor walk does NOT include score in its own early-term comparator — it walks docs in
    ///   the cursor's recorded prefix order.  Within a primary-value tie group, docs are emitted in
    ///   cursor-insertion order; score-aware ordering takes effect only at the cross-segment merge.
    ///   Callers that need exact score-tie ordering for very large tie groups should over-fetch via
    ///   `limit` and accept the well-known ES Lucene `IndexSortByField` + secondary score
    ///   approximation.
    /// * Setting this flag makes the segment scan compute scores for every matched doc (not just
    ///   the K survivors).  When the BM25 cost dominates the query, this is the same total work as
    ///   the legacy heap-based collector — the cursor's early-term win is in the heap-insert path,
    ///   not in the scoring pass itself.
    ...
    ///   * pre-Wave-21: 68% of segment-scan time in `default_collect_segment_impl::{{closure}}`
    ///     (`matched_bitset.insert_docs_batch`), 13% in `AllScorer::fill_buffer`;
    ///   * post-Wave-21 (this flag): both paths collapse to 0% — the walk doesn't run.
    ///
...
///
/// * `Forward` — names AND orders match exactly. Harvest iterates the cursor low→high in stored
///   order; this is the Wave 18 baseline.
/// * `ReverseSingleField` — single-field request whose order is the opposite of the cursor's stored
///   order. Harvest iterates the cursor high→low so the emitted values match the request's order.
///   Multi-field reverse walks would require flipping tie-group lex order which the current harvest
///   doesn't support; we only enable the optimisation for `prefix.len() == 1`.
/// * `NoMatch` — names disagree at some position, the prefix is longer than the cursor's recorded
///   fields, or it's a multi-field request with a mixed-order disagreement we can't reverse-walk.
...
impl Collector for EarlyTermSortByCursorCollectorMulti {
    ...
    fn for_segment(
        ...
        let mut str_columns: Vec<Option<columnar::StrColumn>> = Vec::with_capacity(prefix_len);
        ...
        //
        // * `Exact(p)`               — `Numeric(Some(p))` for both orders (cursor_ord > p in ASC,
        //   cursor_ord < p in DESC).
        // * `Next(0)` + Asc          — `Numeric(None)` (bytes are smaller than every term in dict →
        //   all docs strict-after in ASC).
        // * `Next(num_terms)` + Asc  — `Numeric(Some(u64::MAX))` (bytes are larger than every term
        //   → no docs strict- after; an `> u64::MAX` cmp is always false).
        // * `Next(p)` + Asc, 0<p<n   — `Numeric(Some(p - 1))` (strict-after = cursor_ord ≥ p ⇔
        //   cursor_ord > p - 1).
        // * `Next(p)` + Desc, any p  — `Numeric(Some(p))` (strict-after = cursor_ord < p; Next
        //   means dict has no term at p but the next ord-up is p, so cursor docs with ord < p have
        //   terms < bytes, satisfying DESC strict- after).
        //
...
impl EarlyTermSortByCursorMultiSegmentCollector {
    ...
    ///
    /// * `start_after_raw` is unresolved (`None`) — slow-path bytes-compare fallback handles the
    ///   check per-entry in the harvest loop.
    /// * `prefix_len != 1` — multi-field bisect would need tie-group semantics at the boundary; not
    ///   implemented.
    /// * `start_after_raw[0]` is not a `Numeric(Some(_))` pivot (missing-encoded slot or unresolved
    ///   String).
    /// * The cursor's last stored primary is `None` — indicating the cursor stores
    ///   `missing="_last"` entries that the bisect doesn't bound-check.
    ///
    ...
    fn harvest(self) -> Self::Fruit {
        ...
        for cursor_idx in cursor_iter {
            ...
            if let Some(start_raw) = &self.start_after_raw {
                ...
                let score_opt: Option<f32> = self
                    .scoring
                    .map(|_| self.scores.get(doc as usize).copied().unwrap_or(0.0));
                if !is_strictly_after_lex_raw_inline(
                    &cursor, cursor_idx, prefix_len, score_opt, start_raw, &orders,
                ) {
                    continue;
                }
            }
            ...
            let cur_primary: Vec<Option<u64>> = (0..prefix_len)
...
fn resolve_search_after_for_segment(
    ...
    for (i, slot) in user_start.iter().enumerate() {
        match slot {
            ...
            // forward unchanged.
            CursorSortVal::String(None) | CursorSortVal::Numeric(_) | CursorSortVal::Score(_) => {
                out.push(slot.clone())
            }
        }
    }
...
/// * `start_after[i]` constrained AND `tuple[i]` missing → tuple is "missing" at this depth;
///   missing sorts last so it cannot be "after" a real value (mirrors v1 single-field behaviour).
///   Skip.
/// * `start_after[i]` missing/unconstrained AND `tuple[i]` constrained → tuple is "defined" past
///   where the caller bothered specifying → keep.
/// * Both unconstrained → look at next position.
/// * Both constrained → kind-aware per-field [`Order`] comparison via [`cmp_sort_val`]; if Greater,
///   keep; if Less, drop; if Equal, look at next position.
///
...
mod tests {
    ...
    fn harvest_with_matched_dir(
        ...
        let value_kinds: Vec<ValueKind> = (0..prefix_len).map(|fi| cursor.fields()[fi].2).collect();
    ...
    fn is_strictly_after_lex_raw_inline_parity_with_score_tail() {
        ...
        let cursor = build_cursor(vec![("ts", Order::Desc, ValueKind::I64)], vec![ts], 3);
        let prefix_len = 1;
    ...
    fn single_field_collector_matches_v1_walk() {
        ...
        let v: Vec<Option<u64>> = vec![Some(30i64), Some(10), Some(20)]
            ...
            .collect();
        let cursor = build_cursor(vec![("score", Order::Desc, ValueKind::I64)], vec![v], 4);
    ...
    fn limit_enforces_early_termination() {
        ...
        let cursor = build_cursor(vec![("k", Order::Desc, ValueKind::I64)], vec![v], n);
        let matched: Vec<DocId> = (0..n).collect();
        let hits =
            harvest_with_matched(cursor.clone(), vec![("k", Order::Desc)], 2, None, &matched);
    ...
    fn string_single_field_real_segment_decodes_bytes() {
        use crate::index::{IndexSortByField, SegmentReader};
        ...
        let collector = EarlyTermSortByCursorCollectorMulti::new(vec![("country", Order::Asc)], 10);
    ...
    fn string_cross_segment_dictionary_divergence_uses_bytes() {
        ...
        assert_eq!(
            ...
            "decoded bytes must respect global lex order across segments, not segment-local ord \
             ordering"
        );
    }
    ...
    fn string_then_numeric_multi_field_real_segment() {
        ...
        assert!(matches!(hits[0].0[1], CursorSortVal::Numeric(Some(_))));
    }
    ...
    fn with_scoring_toggles_requires_scoring() {
        let plain = EarlyTermSortByCursorCollectorMulti::new(vec![("ts", Order::Desc)], 10);
        assert!(!plain.requires_scoring());
        assert!(plain.score_order().is_none());

        let scored = EarlyTermSortByCursorCollectorMulti::new(vec![("ts", Order::Desc)], 10)
    ...
    fn cursor_walk_with_scoring_appends_score_slot() {
        ...
        writer
            ...
            .unwrap();
        writer
            .add_document(doc!(body => "java", ts => 150i64))
            .unwrap();
        writer.commit().unwrap();
        let reader = index.reader().unwrap();
        let searcher = reader.searcher();
        ...
        let collector = EarlyTermSortByCursorCollectorMulti::new(vec![("ts", Order::Desc)], 10)
            .with_scoring(Order::Desc);
    ...
    fn merge_fruits_uses_score_as_lex_tail_when_scoring_enabled() {
        ...
        assert_eq!(
            pairs,
            vec![(0, 2), (1, 0), (0, 1), (1, 4)],
            "ts=100 tie group must be ordered by score DESC, with seg 1 doc 0 (score 2.0) before \
             seg 0 doc 1 (score 0.5)"
        );
    }
    ...
    fn wave_18_6_tie_group_buffer_picks_top_score_past_cursor_limit() {
        ...
        for i in 0..5 {
            ...
            writer
                .add_document(doc!(body => text.as_str(), ts => 100i64))
                .unwrap();
        }
        writer.commit().unwrap();
        let reader = index.reader().unwrap();
        ...
        let collector = EarlyTermSortByCursorCollectorMulti::new(vec![("ts", Order::Desc)], 2)
            .with_scoring(Order::Desc);
    ...
    fn wave_18_6_early_break_at_tie_group_boundary() {
        ...
        let collector = EarlyTermSortByCursorCollectorMulti::new(vec![("ts", Order::Desc)], 3)
            .with_scoring(Order::Desc);
        let hits = searcher.search(&*q, &collector).unwrap();
    ...
    fn wave_18_6_no_scoring_preserves_v1_v3_behaviour() {
        ...
        let cursor = build_cursor(vec![("ts", Order::Desc, ValueKind::I64)], vec![v], 4);
        let hits = harvest_with_matched(cursor, vec![("ts", Order::Desc)], 10, None, &[0, 1, 2, 3]);
        let docs: Vec<DocId> = hits.iter().map(|(_, a)| a.doc_id).collect();
    ...
    fn build_string_search_after_test_index(
        terms: &[&str],
        order: Order,
    ) -> (crate::Searcher, crate::schema::Field, crate::schema::Field) {
        use crate::index::IndexSortByField;
        use crate::schema::{Schema, FAST, INDEXED, STORED, STRING};
        use crate::{Index, IndexBuilder, IndexSettings};
        let mut sb = Schema::builder();
    ...
    fn wave_18_3_followup_resolve_desc_exact() {
        ...
        let collector =
            EarlyTermSortByCursorCollectorMulti::new(vec![("country", Order::Desc)], 10)
                .with_search_after(vec![s(b"JP")]);
        let hits = searcher
            .search(&crate::query::AllQuery, &collector)
            .unwrap();
    ...
    fn wave_20_v2_reverse_walk_single_field() {
        ...
        let ts: Vec<Option<u64>> = vec![5i64, 4, 3, 2, 1]
            ...
            .map(|v| Some(v.to_u64()))
            .collect();
        let cursor = build_cursor(vec![("ts", Order::Desc, ValueKind::I64)], vec![ts], 5);
    ...
    fn wave_20_v3_bisect_reverse_walk_asc_with_pivot() {
        ...
        let vs: Vec<Option<u64>> = (0..1000i64).rev().map(|v| Some(v.to_u64())).collect();
        let cursor = build_cursor(vec![("ts", Order::Desc, ValueKind::I64)], vec![vs], 1000);
    ...
    fn wave_20_v3_bisect_fall_through_when_primary_has_missing() {
        ...
        let cursor = build_cursor(vec![("ts", Order::Desc, ValueKind::I64)], vec![vs], 4);
    ...
    fn wave_20_v3_harvest_with_bisect_matches_scan() {
        ...
        let vs: Vec<Option<u64>> = (0..20i64).rev().map(|v| Some(v.to_u64())).collect();
        let cursor = build_cursor(vec![("ts", Order::Desc, ValueKind::I64)], vec![vs], 20);
