...
impl SortCursorIndex {
    ...
    pub fn from_bytes(buf: &[u8]) -> crate::Result<Self> {
        ...
        let trailing_magic = u32::deserialize(&mut reader).map_err(|e| {
            DataCorruption::comment_only(format!("sort cursor: failed to read trailing magic: {e}"))
        })?;
...
impl SortCursorIndexV2 {
    ...
    pub fn from_bytes(buf: &[u8]) -> crate::Result<Self> {
        ...
        let num_fields = u8::deserialize(&mut reader).map_err(|e| {
            DataCorruption::comment_only(format!("sort cursor v2: failed to read num_fields: {e}"))
        })? as usize;
...
impl SortCursorAny {
    ...
    pub fn from_bytes(buf: &[u8]) -> crate::Result<Self> {
        ...
        match buf[4] {
            ...
            SORT_CURSOR_VERSION_V2 => Ok(SortCursorAny::V2(SortCursorIndexV2::from_bytes(buf)?)),
            other => Err(DataCorruption::comment_only(format!(
...
fn read_field_to_u64(
    ...
    if let Some(col) = readers.column_opt::<i64>(field)? {
        let values: Vec<Option<u64>> = (0..num_docs)
            .map(|d| {
                col.first(d)
                    .map(<i64 as MonotonicallyMappableToU64>::to_u64)
            })
            .collect();
    ...
    if let Some(col) = readers.column_opt::<u64>(field)? {
        let values: Vec<Option<u64>> = (0..num_docs)
            .map(|d| {
                col.first(d)
                    .map(<u64 as MonotonicallyMappableToU64>::to_u64)
            })
            .collect();
    ...
    if let Some(col) = readers.column_opt::<f64>(field)? {
        let values: Vec<Option<u64>> = (0..num_docs)
            .map(|d| {
                col.first(d)
                    .map(<f64 as MonotonicallyMappableToU64>::to_u64)
            })
            .collect();
...
mod tests {
    ...
    fn missing_values_sort_last_for_both_orders() {
        ...
        let cursor_asc =
            SortCursorIndex::build_from_values("ts", Order::Asc, vec![Some(5i64), None, Some(1)]);
        assert_eq!(cursor_asc.doc_ids(), &[2u32, 0, 1]);

        let cursor_desc =
            SortCursorIndex::build_from_values("ts", Order::Desc, vec![Some(5i64), None, Some(1)]);
    ...
    fn write_open_roundtrip_large() {
        ...
        let values2: Vec<i64> = (0..n).map(|i| ((i as i64) * 2654435761) & 0xFFFF).collect();
    ...
    fn empty_cursor_roundtrips() {
        let cursor = SortCursorIndex::build_from_values::<i64>("empty", Order::Asc, Vec::new());
    ...
    fn corrupted_magic_is_rejected() {
        let cursor =
            SortCursorIndex::build_from_values("f", Order::Asc, vec![Some(1i64), Some(2), Some(3)]);
        let mut buf = Vec::new();
    ...
    fn unsupported_version_is_rejected() {
        let cursor = SortCursorIndex::build_from_values("f", Order::Asc, vec![Some(1i64), Some(2)]);
        let mut buf = Vec::new();
        cursor.write(&mut buf).unwrap();
    ...
    fn truncated_payload_is_rejected() {
        let cursor =
            SortCursorIndex::build_from_values("f", Order::Asc, (0..256i64).map(Some).collect());
        let mut buf = Vec::new();
        cursor.write(&mut buf).unwrap();
    ...
    fn post_force_merge_segment_doc_ids_match_sort_order() -> crate::Result<()> {
        ...
        assert_eq!(
            ...
            "Phase H-2: merged segment's doc-id sequence must match sort order (identity cursor); \
             without H-2 the order would still be the input-segment concatenation"
        );
    ...
    fn v2_single_field_matches_v1_for_i64_desc() {
        ...
        let v: Vec<Option<u64>> = vec![Some(30i64), Some(10), Some(20)]
            ...
            .collect();
        let cursor = build_v2_from_values(vec![("score", Order::Desc, ValueKind::I64)], vec![v], 4);
    ...
    fn v2_max_fields_cap_enforced_on_build_and_read() {
        ...
        let too_many: Vec<(String, Order, ValueKind)> = (0..9)
            ...
            .collect();
        let too_many_vals: Vec<Vec<Option<u64>>> =
            (0..9).map(|_| vec![Some(0u64), Some(1), Some(2)]).collect();
    ...
    fn v2_string_then_numeric_multi_field_lex_walk() {
        ...
        //   doc 5 → ("JP", missing)
        let country: Vec<Option<u64>> = vec![Some(1), Some(0), Some(1), Some(2), Some(0), Some(1)];
    ...
    fn sort_cursor_any_dispatches_by_version() {
        ...
        let v1 = SortCursorIndex::build_from_values("x", Order::Asc, vec![Some(1i64), Some(2)]);
    ...
    fn rewrite_settings_on_disk_round_trip_v1_to_v2() {
        ...
        writer.add_document(doc!(ts_field => 100i64)).unwrap();
    ...
    fn build_and_write_sort_cursor_v2_for_marks_pending_protects_from_gc() -> crate::Result<()> {
        ...
        assert!(
            ...
            "test setup invariant: pre-publication living-files must exclude the cursor path so \
             the GC race is reproducible"
        );
