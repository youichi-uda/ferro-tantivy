...
//!
//! - **Default** ([`feature = "ferro-compress"`] off): the [`IdentityBitcompCodec`] passes raw word
//!   bytes through, round-tripping cleanly. Useful for tests, the OSS fork, and any non-FerroSearch
//!   downstream.
//! - **Enabled** ([`feature = "ferro-compress"`] on): the [`FerroBitcompCodec`] *will* delegate to
//!   `ferro_compress::nvcomp::Algo::Bitcomp` once Cargo wiring lands in Wave 6 (Phase 2 C-4). Until
//!   then it returns [`BitcompError::WaitingOnWave6`] from every call — by design, so a
//!   misconfigured build fails fast rather than silently skipping compression.
//!
//! ## Wire considerations
//!
...
//! 2. keep the GPU-native form (raw bitmap) and the on-disk form (Bitcomp-compressed bitmap)
//!    decoupled — the encoder writes `compress(raw_words)` to disk and the decoder reads
//!    `decompress(compressed_bytes) -> [u32; 2048]` back into VRAM / host memory.
//!
//! ## What this wave (C-3) does *not* do
//!
//! - No `Cargo.toml` `path = "../ferro-compress"` dependency wiring (Wave 6 / C-4).
//! - No `Cargo.lock` mutation outside this crate.
//! - No `BitmapContainer::compressed_bytes` helpers on the container itself (kept here, in the
//!   bridge module, to avoid a circular dep direction between `container.rs` and the codec layer).
...
/// - With `ferro-compress` **on**: [`FerroBitcompCodec`] (which still short-circuits to
///   [`BitcompError::WaitingOnWave6`] until C-4).
